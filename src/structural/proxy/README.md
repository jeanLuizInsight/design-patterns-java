...
Um exemplo do mundo real, imagine uma torre onde os magos locais vão estudar seus feitiços. A torre de marfim só pode ser acessada através de um proxy que garante que apenas os três primeiros assistentes possam entrar. Aqui o proxy representa a funcionalidade da torre e adiciona controle de acesso a ela. Simplificando, usando o padrão de proxy, uma classe irá representar a funcionalidade de outra classe.

Primeiramente temos a interface `TorreDoMago` e sua classe de implementação `TorreDeMarfim`. Então uma classe simples `Mago` pra sua representação. Em seguida é criado o proxy `TorreDoMagoProxy` controle de acesso para adicionar a `TorreDoMago`.
